...
#TROIS PETITES FONCTIONS DE TEST UTILISÉES PLUS BAS#
...
def retablir_for(formule_init, list_chgmts):
    ...
    un ensemble de variables (chaque changement étant une liste [i,bool] avec i 
    l'index qu'occupe la variable dans list_var et bool la valeur logique qui doit 
    lui être assignée) 
...
def progress_simpl_for_dpll(formule, list_var, list_chgmts, list_sans_retour):
    ...
    print(new_list_chgmts, list_chgmts)
    if new_list_chgmts != list_chgmts:
        list_sans_retour.append(new_list_chgmts[len(new_list_chgmts)-1][0])
        print(list_sans_retour)
...
# test de resol_parcours_arbre_simpl_for
# formule_init= [[1, 2, 4, -5], [-1, 2, 3, -4], [-1, -2, -5], [-3, 4, 5], [-2, 3, 4, 5], [-4, 5]] 
...
# test de resol_parcours_arbre_simpl_for_dpll
# formule_init= [[1, 2, 4, -5], [-1, 2, 3, -4], [-1, -2, -5], [-3, 4, 5], [-2, 3, 4, 5], [-4, 5]] 
# formule= [[2, 3, -4], [-2, -5], [-3, 4, 5], [-2, 3, 4, 5], [-4, 5]] 
# list_var= [True, None, None, None, None] 
# list_chgmts= [[0, True]] 
...
def creer_grille_init(list_grille,n):
    '''
    Arguments : une liste de listes(contenant les coordonnées à renseigner 
    et le nombre correspondant) et un entier donnant la taille de la grille
    Renvoie : une liste (list_grille_complete) avec les valeurs qui devront 
    s'afficher dans la grille en la parcourant ligne après ligne de haut 
    en bas et de gauche à droite
    '''

# test pour creer_grille_init
...
def creer_grille_final(list_var,n):
    '''
    Renvoie : une liste (list_grille_complete) avec les valeurs qui 
    devront s'afficher dans la grille (en fonction des valeurs logiques 
    prises par les variables de list_var) en la parcourant ligne après 
    ligne de haut en bas et de gauche à droite
    '''

# test pour creer_grille_final
...
def for_conj_sudoku(n):
    '''
    Renvoie : la formule (liste de listes) associée à une grille de sudoku 
    de taille n selon les attentes formulées dans le sujet
    '''

# test pour for_conj_sudoku
...
def init_list_var(grille,n):
    '''
    Renvoie : une liste list_var initialisant une valuation 
    tenant compte des valeurs non nulles déjà renseignées dans 
    list_grille_complete
    '''

# test pour init_list_var
