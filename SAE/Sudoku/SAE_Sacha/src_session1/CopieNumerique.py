...
def evaluer_clause(clause, list_var):
    ...
    for i in clause:
        currentVarPos = abs(i)
        currentVar = getI(currentVarPos, list_var)
        ...
        resultat = currentVar or resultat
...
def evaluer_cnf(formule,list_var):
    ...
    resultat = True
    for i in formule:
        resClause = evaluer_clause(i, list_var)
        if resClause == None:
            return None
        else :
            resultat = resultat and resClause
    return resultat
...
def determine_valuations(list_var):
    ...
    if None in list_var:
        ...
        l1 = copy.deepcopy(list_var)
        l2 = copy.deepcopy(list_var)
        l1[nextNone] = True
        l2[nextNone] = False
        listValuations += determine_valuations(l1)
        listValuations += determine_valuations(l2)

        #print(listValuations)
...
def resol_sat_force_brute(formule, list_var):
    ...
    '''
    listValuations = determine_valuations(list_var)
    for i in listValuations:
        resultat = evaluer_cnf(formule, i)
        if resultat :
            return resultat, i
    return resultat, []
...
def enlever_litt_for(formule, litteral):
    ...
    #         if abs(i) == litteral:
    #             if i>0 :
    #                 clauseToPop.append(clause)
    #                 # print(">0")
    #             elif i<0 :
    #                 clause.pop(clause.index(i))
    #                 # print("<0")
    # for i in clauseToPop:
    #     formule.pop(formule.index(i))
    # # print(formule)
...
def init_formule_simpl_for(formule_init, list_var):
    ...
    '''
    # Code de la session n1
    #print(formule_init)
    # for i in range(len(list_var)):
    #     currentVar = list_var[i]
    #     litteral = i+1
    #     if currentVar:
    #         formule_init = (enlever_litt_for(formule_init, litteral))
    #         print(litteral, formule_init)
    #     elif currentVar == False:
    #         # cause erreur dans test 1 -> calcule à faire si currentVar = False ?
    #         formule_init = (enlever_litt_for(formule_init, litteral)) 
    #         print(litteral, formule_init)
...
def retablir_for(formule_init, list_chgmts):
    '''
    Arguments : une formule initiale et une liste de changements à 
    apporter sur un ensemble de variables (chaque changement étant 
    une liste [i,bool] avec i l'index qu'occupe la variable dans 
    list_var et bool la valeur logique qui doit lui être assignée) 
    Renvoie : la formule simplifiée en tenant compte de l'ensemble 
    des changements
    '''
...
def progress_simpl_for(formule, list_var, list_chgmts):
    ...
    l2 : nouvelle list_chgmts 
    '''
    # pendant l'évaluation : directement coder progress ici
...
def retour_simpl_for(formule_init, list_var, list_chgmts):
    ...
    l2 : nouvelle list_chgmts 
    '''
    # pendant l'évaluation : directement coder retour ici
    list_var, list_chgmts = retour(list_var, list_chgmts)
